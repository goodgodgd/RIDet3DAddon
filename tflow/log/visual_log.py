...
class VisualLog:
    ...
    def __call__(self, step, grtr, pred):
        ...
        """
        self.visual(step, grtr, pred)

    def visual(self, step, grtr, pred):
        splits = {}
...
#         grtr_bbox = {}
#         grtr_bbox["inst3d"] = self.exapand_grtr_bbox_3d(grtr, pred)
#         grtr_bbox["inst2d"] = self.exapand_grtr_bbox_2d(grtr, pred)
#         splits = split_tp_fp_fn_3d(grtr_bbox, pred, cfg.Validation.TP_IOU_THRESH)
#         batch = splits["grtr_tp"]["yx"].shape[0]
#
#         for i in range(batch):
#             # grtr_log_keys = ["pred_object", "pred_ctgr_prob", "pred_score", "distance"]
#             image_grtr = uf.to_uint8_image(grtr["image"][i]).numpy()
#             # bev_image = np.zeros((500, 1200, 3), dtype=np.uint8)
#             bev_image = np.zeros((800, 500, 3), dtype=np.uint8)
...
#         proj_box, proj_box_center, bev_box = self.extract_corner(intrinsic[frame_idx], yx, z, hwl, theta)
#
#         front_view = self.draw_cuboid(image, proj_box, proj_box_center, category, z, tp_fp_color)
#
