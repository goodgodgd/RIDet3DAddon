...
class VisualLog3d(VisualLog):
    ...
    def visual_3d(self, step, grtr, pred):
        ...
        batch = splits["grtr_tp"]["yxz"].shape[0]

        for i in range(batch):
            # grtr_log_keys = ["pred_object", "pred_ctgr_prob", "pred_score", "distance"]
            image_grtr = uf.to_uint8_image(grtr["image"][i]).numpy()
            bev_image = np.zeros((800, 500, 3), dtype=np.uint8)
            image_grtr, bev_image = self.draw_boxes(image_grtr, bev_image, splits["grtr_tp"], i, self.grtr_log_keys,
                                                    (0, 255, 0), (255, 0, 0), grtr["intrinsic"])
            image_grtr, bev_image = self.draw_boxes(image_grtr, bev_image, splits["grtr_fn"], i, self.grtr_log_keys,
                                                    (0, 0, 255), (255, 0, 0), grtr["intrinsic"])
            image_pred = uf.to_uint8_image(grtr["image"][i]).numpy()
            image_pred, bev_image = self.draw_boxes(image_pred, bev_image, splits["pred_tp"], i, [], (0, 255, 0),
                                                    (0, 0, 255), grtr["intrinsic"])
            image_pred, bev_image = self.draw_boxes(image_pred, bev_image, splits["pred_fp"], i, [], (0, 0, 255),
                                                    (0, 0, 255), grtr["intrinsic"])
    ...
    def draw_boxes(self, image, bev_image, bboxes, frame_idx, log_keys, tp_fp_color, gt_pr_color, intrinsic):
        category = bboxes["category"][frame_idx]  # (N, 1)
        valid_mask = bboxes["hwl"][frame_idx][:, 0] > 0  # (N,) h>0
        yxz = bboxes["yxz"][frame_idx][valid_mask]
        ...
        proj_box, proj_box_center, bev_box = self.extract_corner(intrinsic[frame_idx], yxz, hwl, theta)

        front_view = self.draw_cuboid(image, proj_box, proj_box_center, category, yxz, tp_fp_color)

        bev_view = self.draw_bev(bev_image, bev_box, category, gt_pr_color, iou)

        return front_view, bev_view

    def extract_corner(self, intrinsic, yxz, hwl, theta):
        proj_box = list()
        proj_box_center = list()
        bev_box = list()
        for i in range(yxz.shape[0]):
            rot = theta[i]
            y = yxz[i, 0]
            x = yxz[i, 1]
            z = yxz[i, 2]
            ...
            for i in [1, -1]:
                for j in [1, -1]:
                    for k in [0, 1]:
                        ...
                        point[2] = z + i * w / 2 * np.sin(-rot + np.pi / 2) + (j * i) * l / 2 * np.sin(-rot)
                        point[1] = y - k * h
    ...
    def draw_cuboid(self, image, proj_box, proj_box_center, category, yxz, color):
