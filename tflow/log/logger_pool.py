...
class LogMeanDetailLoss(LogBase):
    def __call__(self, grtr, pred, loss):
        ...
        grtr_ctgr_mask = self.one_hot(grtr["feat"]["category"], self.num_categs)    # (b, h*w, anchor, 1) -> (b, h*w, anchor, category)
        grtr_tgarget_mask = grtr_ctgr_mask * grtr["feat"]["object"]
        nonan = (1 - grtr["feat"]["object"]) * 1e-12
...
class LogPositiveObj(LogBase):
    def __call__(self, grtr, pred, loss):
        ...
        for scale in range(len(grtr["feat"]["object"])):
            ...
            pos_obj += pos_obj_sc
        return pos_obj / len(grtr["feat"]["object"])
...
class LogNegativeObj(LogBase):
    def __call__(self, grtr, pred, loss):
        ...
        for scale in range(len(grtr["feat"]["object"])):
            ...
            neg_obj += neg_obj_sc
        return neg_obj / len(grtr["feat"]["object"])
...
class LogPositiveDetailObj(LogBase):
    def __call__(self, grtr, pred, loss):
        grtr_ctgr_mask = self.one_hot(grtr["feat"]["category"], self.num_categs)
        grtr_target_mask = grtr_ctgr_mask * grtr["feat"]["object"]
        nonan = (1 - grtr["feat"]["object"]) * 1e-12
        pos_obj_num = np.sum(grtr_target_mask + nonan, axis=(0, 1), dtype=np.float32)
        pos_obj = np.sum(grtr_target_mask * pred["feat"]["object"], axis=(0, 1), dtype=np.float32) / pos_obj_num
        return np.reshape(pos_obj, -1).tolist()


class LogNegativeDetailObj(LogBase):
    def __call__(self, grtr, pred, loss):
        best_pred_ctgr = np.expand_dims(np.argmax(pred["feat"]["category"], axis=-1), axis=-1)
        target_ctgr_mask = self.one_hot(best_pred_ctgr, self.num_categs)
        neg_obj_mask = 1. - grtr["feat"]["object"]
        neg_obj_map = neg_obj_mask * pred["feat"]["object"] * target_ctgr_mask
        batch, hw, a, channel = grtr["feat"]["object"].shape
        neg_obj_list = []
...
class LogIouMean(LogBase):
    def __call__(self, grtr, pred, loss):
        grtr_ctgr_mask = self.one_hot(grtr["feat"]["category"], self.num_categs)
        grtr_obj_mask = grtr_ctgr_mask * grtr["feat"]["object"]
        grtr_yxhw = np.expand_dims(grtr["feat"]["yxhw"], axis=-2) * np.expand_dims(grtr_obj_mask, axis=-1)  # (b, h*w, anchor,category,yxhw)
        pred_yxhw = np.expand_dims(pred["feat"]["yxhw"], axis=-2) * np.expand_dims(pred["feat"]["category"], axis=-1)
...
class LogTrueClass(LogBase):
    def __call__(self, grtr, pred, loss):
        ...
        category_prob = grtr_target_mask * pred["feat"]["category"]
...
class LogFalseClass(LogBase):
    def __call__(self, grtr, pred, loss):
        ...
        false_prob = false_ctgr_mask * pred["feat"]["category"] * grtr["feat"]["object"]
...
class LogBoxYXZ(LogBase):
    def __call__(self, grtr, pred, loss):
        grtr_ctgr_mask = self.one_hot(grtr["feat"]["category"], self.num_categs)
        grtr_obj_mask = grtr_ctgr_mask * grtr["feat"]["object"]
        grtr_yxz = np.expand_dims(grtr["feat"]["yxz"], axis=-2) * np.expand_dims(grtr_obj_mask,
                                                                           axis=-1)  # (b, h*w, anchor,category,yxz)
        pred_yxz = np.expand_dims(pred["feat"]["yxz"], axis=-2) * np.expand_dims(pred["feat"]["category"], axis=-1)
...
class LogBoxHWL(LogBase):
    def __call__(self, grtr, pred, loss):
        grtr_ctgr_mask = self.one_hot(grtr["feat"]["category"], self.num_categs)
        grtr_obj_mask = grtr_ctgr_mask * grtr["feat"]["object"]
        grtr_hwl = np.expand_dims(grtr["feat"]["hwl"], axis=-2) * np.expand_dims(grtr_obj_mask,
                                                                           axis=-1)  # (b, h*w, anchor,category,yxhw)
        pred_hwl = np.expand_dims(pred["feat"]["hwl"], axis=-2) * np.expand_dims(pred["feat"]["category"], axis=-1)

        hwl_diff = (np.abs(grtr_hwl - pred_hwl, dtype=np.float32)) / (grtr_hwl + 1e-12) * np.expand_dims(pred["feat"]["category"], axis=-1)
