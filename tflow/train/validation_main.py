...
def validate_main():
    uf.set_gpu_configs()
    ckpt_path = op.join(cfg3d.Paths.CHECK_POINT, cfg3d.Train.CKPT_NAME)
    latest_epoch = read_previous_epoch(ckpt_path)
    val_epoch = cfg3d.Validation.VAL_EPOCH
...
def analyze_performance(dataset_name, loss_weights, weight_suffix, latest_epoch):
    ...
    datapath, ckpt_path = cfg3d.Paths.DATAPATH, op.join(cfg3d.Paths.CHECK_POINT, cfg3d.Train.CKPT_NAME)
