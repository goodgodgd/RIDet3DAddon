...
class ExamplePreprocess(PreprocessBase):
    def __init__(self, target_hw, dataset_cfg, max_bbox, max_dontcare, min_pix, category_names):
...
class ExampleCropper(PreprocessBase):
    ...
    def crop_bboxes(self, bboxes, crop_tlbr, cropped_hw):
        ...
        if bboxes.size == 0:
            raise uc.MyExceptionToCatch(f"[get_2d_box] empty boxes{test_bboxe}")
...
class ExampleCategoryRemapper(PreprocessBase):
    ...
    def __call__(self, example):
        ...
        example["inst2d"] = example["inst2d"][new_categs != self.INVALID_CATEGORY, :]
        return example
