...
class EvaluateNmsParams:
    ...
    def load_dataset_model(self, dataset_name, ckpt_path):
        ...
        dataset, steps, imshape, anchors_per_scale \
            = self.get_dataset(datapath, dataset_name, False, batch_size, "val", anchors)
    ...
    def get_dataset(self, datapath, dataset_name, shuffle, batch_size, split, anchors):
    ...
    def collect_recall_precision(self, dataset, steps, model, num_ctgr, anchors_per_scale, feature_creator):
        ...
        nms_ = nms.NonMaximumSuppression()
        for step, grtr in enumerate(dataset):
            frame_names = grtr['frame_names']
            del grtr["frame_names"]
            grtr = feature_creator(grtr)
            image = uf.convert_to_tensor(grtr['image'], "float32", True)
            image = self.normalizer(image.permute(0, 3, 1, 2).to(self.device))
